---
layout: default
title: "Home"
permalink: /
class: home
---

<div class="home">

  <!-- HERO SECTION -->
  <section class="forge-hero-vaporized">

    <!-- TAGLINE -->
    <p class="hero-tagline forge-text-gradient">
      Practical, relationship-focused tech consulting for homes, churches,
      libraries, and small businesses.
    </p>

    <!-- BRAND STAMP -->
    <div class="hero-stamp">
      M.R. TechForge
    </div>

    <!-- HERO TITLE -->
    <h1 class="hero-title">
      Crafting Practical<br>
      Tech Solutions<br>
      With Integrity & Skill
    </h1>

    <!-- SUBTITLE -->
    <p class="hero-subtitle forge-text-gradient">
      Clear, reliable, and relationship-driven technology help for your home,
      small business, church, or community. Built on hands-on experience, a love
      for Linux, and a commitment to integrity.
    </p>

    <!-- TAGS -->
    <div class="hero-tags">
      <span class="hero-tag">Linux</span>
      <span class="hero-tag">Digital Security</span>
      <span class="hero-tag">Networking</span>
      <span class="hero-tag">Web Services</span>
      <span class="hero-tag">Customized Tech Gadgets</span>
    </div>

    <!-- BUTTONS -->
    <div class="hero-actions">
      <a href="/services/" class="btn btn-forge">Our Services</a>
      <a href="/contact.html" class="btn btn-forge">Get in Touch</a>
    </div>

    <!-- FOOTNOTE -->
    <div class="hero-footnote forge-text-gradient">
      Forging relationships — and solutions — that last.
    </div>
  </section>


  <!-- SPACER -->
  <div class="forge-section-spacer"></div>


  <!-- WHAT WE STAND FOR -->
  <section class="home-pillars">
    <h2>What We Stand For</h2>

    <div class="pillar-grid">

      <div class="pillar-card">
        <h3>Clarity</h3>
        <p class="forge-text-gradient">
          No confusing jargon. Clear explanations and real guidance.
        </p>
      </div>

      <div class="pillar-card">
        <h3>Reliability</h3>
        <p class="forge-text-gradient">
          Dependable setups built to last — not quick hacks or shortcuts.
        </p>
      </div>

      <div class="pillar-card">
        <h3>Integrity</h3>
        <p class="forge-text-gradient">
          Faith-driven values. Honest recommendations. Work done the right way.
        </p>
      </div>

    </div>
  </section>


  <!-- SPACER -->
  <div class="forge-section-spacer"></div>


  <!-- WHO I HELP -->
  <section class="home-audience">
    <h2>Who I Help</h2>

    <div class="audience-grid">

      <div class="pillar-card">
        <ul>
          <li class="forge-text-gradient">Families needing safer, cleaner digital setups</li>
          <li class="forge-text-gradient">Small businesses seeking dependable tech guidance</li>
          <li class="forge-text-gradient">Churches wanting trustworthy network or communication help</li>
          <li class="forge-text-gradient">Customized tech gadgets needing integration, automation, or secure routing</li>
        </ul>
      </div>

      <div class="pillar-card">
        <ul>
          <li class="forge-text-gradient">Professionals needing privacy-focused technical solutions</li>
          <li class="forge-text-gradient">Anyone overwhelmed by “big tech” systems and noise</li>
          <li class="forge-text-gradient">People wanting simpler, more organized home tech</li>
          <li class="forge-text-gradient">Those ready to build better digital habits</li>
        </ul>
      </div>

    </div>
  </section>


  <!-- SPACER -->
  <div class="forge-section-spacer"></div>


  <!-- WHAT'S COMING SOON -->
  <section class="home-products">
    <h2>What’s Coming Soon</h2>

    <div class="pillar-card">
      <p class="forge-text-gradient">
        M.R. TechForge is actively developing a handful of lightweight,
        user-friendly tools designed to make digital life safer, calmer, and easier.
      </p>

      <ul>
        <li class="forge-text-gradient">
          <strong>ForgeDrive:</strong> A clean, organized USB toolkit with utilities & documentation
        </li>
        <li class="forge-text-gradient">
          <strong>Forge Tags:</strong> Simple NFC cards for easy, secure information sharing
        </li>
        <li class="forge-text-gradient">
          <strong>ForgeSBC:</strong> A Raspberry-Pi powered home hub for ad-blocking, secure access, and syncing
        </li>
      </ul>

      <p class="forge-text-gradient">
        These tools are optional add-ons — helpful if you want a polished, secure foundation
        for your devices.
      </p>
    </div>
  </section>


  <!-- SPACER -->
  <div class="forge-section-spacer"></div>


  <!-- HOW IT WORKS -->
  <section class="home-how">
    <h2>How It Works</h2>

    <ul class="how-steps">

      <li class="pillar-card">
        <span class="step-label">1. Connect</span>
        <span class="forge-text-gradient">
          We talk through your needs, challenges, and goals — in plain English.
        </span>
      </li>

      <li class="pillar-card">
        <span class="step-label">2. Plan</span>
        <span class="forge-text-gradient">
          You get clear, practical options instead of overwhelming complexity.
        </span>
      </li>

      <li class="pillar-card">
        <span class="step-label">3. Build</span>
        <span class="forge-text-gradient">
          Clean, dependable solutions designed for long-term stability.
        </span>
      </li>

    </ul>
  </section>


  <!-- SPACER -->
  <div class="forge-section-spacer"></div>


  <!-- INTEGRITY SECTION -->
  <section class="home-integrity">
    <h2>Faith • Integrity • Craftsmanship</h2>

    <div class="pillar-card">
      <p class="forge-text-gradient">
        Every project is guided by values — honesty, transparency, and a commitment
        to treating people right. Whether it’s a consultation or a full setup, you
        get dependable help rooted in faith-driven integrity.
      </p>
    </div>
  </section>

</div>
